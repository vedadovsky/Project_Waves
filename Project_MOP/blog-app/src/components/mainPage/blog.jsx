...
<BodyForm>
    <HeaderForm>
        <Image></Image>
        <UserNav onClick={myFunction}>User: {currentUser}</UserNav>
        ...
        <BlogContainer>
            ...
            {postList.map((val,key)=>{
             return( 
                <BlogInfo >
            <BlogLikes>
                <LikerUp onClick={()=>{likePost(val.id)}}><FaArrowAltCircleUp/></LikerUp>
                <Text>{val.likes}</Text>
                <LikerDown onClick={()=>{disLike(val.id)}}><FaArrowAltCircleDown/></LikerDown>
            </BlogLikes>
